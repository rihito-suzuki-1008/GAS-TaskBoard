...
**リストビュー**：名称セル内（編集アイコンの隣）に「＋」アイコン（2.1のGhostボタン相当）を表示する。さらに、展開中の親ノードの子リスト末尾に「＋ タスクを追加」のゴースト行を常設する。どちらもクリックすると、その親の子ノードとして即座にshellを`addNode`し、破線境界のプレースホルダー行として挿入する。同時にタスク詳細ドロワー（2.2a）が開き、名称入力欄に自動でフォーカスが当たった状態にする。「作成」ボタンによる明示的な確定操作は挟まない。依存関係を持つ末端ノードには「＋」もゴースト行も表示しない。

**ガントビュー**：子を作る操作と期間を描く操作を分離する。左ツリーはリストビューと同じく、名称セル内の「＋」とゴースト行で子shellを作成する。タイムライン背景のドラッグは行の状態で意味を変える。日付未設定の末端ノード行では、そのノード自身のStartDate/EndDateを設定してバーを描く。子を持つノード行・ゴースト行・既存行下の空レーンでは、指定期間付きの子shellを作成する。日付設定済みの末端ノード行では背景ドラッグによる作成は行わない。ドラッグ中は日付チップを表示し、Escでキャンセルできる。
