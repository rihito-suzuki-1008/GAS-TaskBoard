...
- 親コメントは通常のコメントカードとして表示する。
- 返信コメントは `--space-6` 程度インデントし、左端に `--space-1` 幅の `--color-border-strong` ボーダーを置く。
- コメントヘッダにはメンバー色のアバター、投稿者名、投稿日時を表示する。メンバーが削除済みの場合は `--color-text-tertiary` のグレーアバターにする。
- 自分宛メンションを含むコメントは `--color-accent-subtle` 背景で強調する。
